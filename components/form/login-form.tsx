...
import { signInSchema, type SignInSchema } from '@/zod/auth'
...
export function LoginForm() {
  ...
  async function onSubmit(values: SignInSchema) {
    ...
    setPending(false)
    if (error) {
      form.setError('root', { message: error.message || 'Неуспешен вход.' })
      return
    }
    router.push('/')
    router.refresh()
  ...
  return (
    <div className="mx-auto flex w-full max-w-[min(100%,380px)] flex-col items-center px-1">
      ...
      <h1 className="mb-1 text-center text-2xl font-semibold tracking-tight sm:text-3xl">Здравейте!</h1>
      <p className="mb-8 text-center text-sm text-muted-foreground">Моля, въведете Вашите данни.</p>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-4">
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex justify-end pt-0.5">
                  <Link href="/forgot-password" className={`text-sm font-medium ${accent}`}>
                    Забравена парола?
                  </Link>
                </div>
                <FormMessage />
          ...
          <Button
            type="submit"
            disabled={pending}
            className="h-12 w-full rounded-full border-0 bg-black text-base font-medium text-white hover:bg-black/90"
          >
      ...
      <p className="mt-10 text-center text-sm ">
        Не сте регистриран?{' '}
        <Link href="/auth/register" className={`font-medium ${accent}`}>
          Регистрация
        </Link>
      </p>
